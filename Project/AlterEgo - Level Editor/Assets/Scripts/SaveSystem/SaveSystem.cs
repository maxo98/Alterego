...
public class SaveSystem : MonoBehaviour
{
    ...
    string directoryPath = "C:/Users/Erwan/AppData/LocalLow/DefaultCompany/3rdPersonPrototype/SaveData/";

    public void SaveData()
    {
        GetMapIntoList();
        SaveManager.Save(SaveManager.editorFilename, directoryPath);
    }

    public void LoadData()
    {
        so = SaveManager.Load(level, SaveManager.editorFilename, directoryPath);
        DestroyOld(level);
        GetListIntoMap(so);
    }
